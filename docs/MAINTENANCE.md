...
- 後台「報名管理」整併進「報名資訊」列表列操作。
  - 若活動已有報名資料：顯示「查詢報名紀錄」按鈕，可開啟報名紀錄彈窗。
  - 若活動尚無報名資料：顯示「新增紀錄」按鈕，直接開啟新增報名表單。
- 報名紀錄彈窗支援原報名管理功能：搜尋、新增、編輯、刪除、下載 Excel。
- 後台左側選單新增收合/展開功能，收合時保留圖示導覽與 tooltip。
- 後台側欄移除獨立「報名管理」入口，操作集中在「報名資訊」。
...
- 報名資訊編輯表單的「類別」欄位改為必選，新增「請選擇類別」占位選項；未選擇時不可保存，避免前台被歸到「其他」。
- 後台報名資訊類別支援拖拉排序，排序後會即時寫回 `activityCategories`，前台分類顯示順序會同步。
- 類別管理支援新增、編輯、刪除；類別改名/刪除會同步更新既有活動資料的 `category` 值。
- 前台報名資訊頁改為按類別分組顯示；每個類別區塊預設展開，使用者可點標題收合。
- 前台分組補強：若資料中有不在設定清單內的類別，會自動補到分組清單，避免內容被隱藏。
- 程式整理：移除未使用程式碼。
