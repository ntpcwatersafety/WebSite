...
- 報名資料管理從獨立頁改為整合至 `AdminActivities` 列表列操作。
- `SortableGalleryList` 支援列級 actions；每列依報名筆數顯示「查詢報名紀錄」或「新增紀錄」。
- 報名紀錄彈窗元件為 `components/admin/ActivityRegistrationsDialog.tsx`，包含新增、編輯、刪除、Excel 匯出。
- 後台左側選單在 `AdminDashboard` 新增收合/展開狀態，收合時保留圖示導覽。
